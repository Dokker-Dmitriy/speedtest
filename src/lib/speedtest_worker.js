...
let settings = {
    ...
    test_order: "IP_D_U", // порядок выполнения тестов в виде строки. D=Скачивание, U=Загрузка, P=Пинг+джиттер, I=IP, _=задержка 1 секунда
    time_ul_max: 10, // максимальная длительность теста загрузки в секундах
    time_dl_max: 10, // максимальная длительность теста скачивания в секундах
    time_auto: true, // если true, тесты будут занимать меньше времени на более быстрых соединениях
    time_ulGraceTime: 3, // время ожидания в секундах перед измерением скорости загрузки (ожидание заполнения буферов)
    time_dlGraceTime: 2, // время ожидания в секундах перед измерением скорости скачивания (ожидание увеличения TCP-окна)
    count_ping: 10, // количество пингов для выполнения в тесте пинга
    url_dl: "http://172.24.6.13/backend/garbage.php", // путь к большому файлу или garbage.php, используемый для теста скачивания. должен быть относительным к этому js-файлу
    url_ul: "http://172.24.6.13/backend/empty.php", // путь к пустому файлу, используемый для теста загрузки. должен быть относительным к этому js-файлу
    url_ping: "http://172.24.6.13/backend/empty.php", // путь к пустому файлу, используемый для теста пинга. должен быть относительным к этому js-файлу
    url_getIp: "http://172.24.6.13/backend/getIP.php", // путь к getIP.php относительно этого js-файла, или аналогичный инструмент, выводящий IP клиента
    getIp_ispInfo: true, // если true, сервер будет включать информацию об ISP вместе с IP-адресом
    getIp_ispInfo_distance: "km", // км или ми=оценка расстояния от сервера в км/милях; установить false для отключения оценки расстояния. getIp_ispInfo должен быть включен
    xhr_dlMultistream: 10, // количество потоков скачивания (может отличаться, если enable_quirks активен)
    xhr_ulMultistream: 5, // количество потоков загрузки (может отличаться, если enable_quirks активен)
    xhr_multistreamDelay: 1000, // задержка между одновременными запросами в миллисекундах
    xhr_ignoreErrors: 2, // 0=сбой при ошибках, 1=попытка перезапустить поток при сбое, 2=игнорировать все ошибки
    xhr_dlUseBlob: false, // если true, уменьшает использование RAM, но использует жесткий диск (полезно с большим garbagePhp_chunkSize и/или высоким xhr_dlMultistream)
    xhr_ul_blob_megabytes: 10, // размер в мегабайтах загружаемых блобов, отправляемых в тесте загрузки (принудительно 4 на мобильном Chrome)
    garbagePhp_chunkSize: 10, // размер чанков, отправляемых garbage.php (может отличаться, если enable_quirks активен)
    enable_quirks: true, // включить особенности для конкретных браузеров. в настоящее время переопределяет настройки для оптимизации под определенные браузеры, если они не переопределены командой start
    ping_allowPerformanceApi: true, // если включено, тест пинга будет пытаться вычислить пинг более точно с использованием Performance API. Работает идеально в Chrome, плохо в Edge, не работает в Firefox. Если API не поддерживается или результат явно неверный, используется запасной вариант
    overheadCompensationFactor: 1.1, // может быть изменен для компенсации накладных расходов на передачу. (см. doc.md для других значений)
    useMebibits: false, // если true, скорость будет отображаться в мебибитах/с вместо мегабит/с
    telemetry_level: 0, // 0=отключено, 1=базовый (только результаты), 2=полный (результаты и время), 3=отладка (результаты+лог)
    url_telemetry: "http://172.24.6.13/results/telemetry.php", // путь к скрипту, добавляющему телеметрические данные в базу данных
    telemetry_extra: "", // дополнительные данные, которые могут быть переданы в телеметрию через настройки
    forceIE11Workaround: false // если true, заставит использовать обходной путь IE11 для теста загрузки во всех браузерах. Только для отладки
};
